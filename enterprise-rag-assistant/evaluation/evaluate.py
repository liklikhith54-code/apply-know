...
async def main():
    ...
    if args.mode in ("improved", "all"):
        ...
        improved_path = os.path.join(RESULTS_DIR, "improved_results.json")
        with open(improved_path, "w", encoding="utf-8") as f:
            json.dump(improved_metrics, f, indent=2)
        logger.info(f"Saved improved results to {improved_path}")

    # Load results from file if they were not run in the current session
    if args.mode == "baseline":
        try:
            ...
            logger.warning(f"Could not load improved results: {e}")
    ...
    if baseline_metrics and improved_metrics:
        ...
        logger.info(f"Saved comparison JSON to {comparison_json_path}")

        # Generate results table (comparison.md & evaluation_report.md)
...
"""
        ...
        logger.info(f"Evaluation report created successfully at: {report_path}")
